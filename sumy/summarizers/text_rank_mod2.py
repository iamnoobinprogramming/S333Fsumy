...
class TextRankSummarizermod(AbstractSummarizer):
    ...
    def __init__(self, stemmer=null_stemmer, alpha=1, rate=0):
        super(TextRankSummarizermod, self).__init__(stemmer)
        self.alpha = alpha
        self.rate = rate
    ...
    def _create_matrix(self, document):
        ...
        for i, words_i in enumerate(sentences_as_words):
            for j in range(i, sentences_count):
                ...
                if (rating < self.rate) and (rating > 0.2):
                    rating *= self.alpha
                    weights[i, j] = rating
                    weights[j, i] = rating
